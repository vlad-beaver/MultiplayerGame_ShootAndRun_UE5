...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		HUD = HUD == nullptr ? Cast<ASarHUD>(Controller->GetHUD()) : HUD;
		if (HUD)
		{
			if (EquippedWeapon)
			{
			...
			if (bAiming)
			{
				CrosshairAimFactor = FMath::FInterpTo(CrosshairAimFactor, 0.58f + CrosshairShrinkAimAtPlayer, DeltaTime, 30.f);
			}
			else
			{
